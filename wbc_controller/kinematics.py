...
class RobotIK:
    ...
    def computeIK(self, foot_pos, foot_vel_= None):
        joint_pos = np.zeros(12)
        joint_vel = np.zeros(12)
        foot_vel = foot_vel_ if foot_vel_ is not None else np.zeros(12)
        for i in range(4):
        ...
        if foot_vel_ is None:
            return joint_pos
        else:
            return joint_pos, joint_vel

Go2_IK = RobotIK(Go2)
Aliengo_IK = RobotIK(Aliengo)
A1_IK = RobotIK(A1)
            
if __name__ == "__main__":
    s = IK(Aliengo,'LH')
    jp,jv = s.compute_inverse(np.array([-0.2399, 0.134, -0.3]),np.array([0,0,0]))
    print(jp,jv)
    
    aliengo_ri = Aliengo_IK
    jp,jv = aliengo_ri.computeIK(np.array([0.2399, 0.134, -0.3,0.2399, -0.134, -0.3,-0.2399, 0.134, -0.3,-0.2399, -0.134, -0.3]),
