...
kp = 10.0                 # proportional gain of end effector task
...
contact_surface_name = "tool0"
contact_surface_pos = np.array([0,0,0])
contact_normal = np.array([0,0,1])
K = np.diag(np.ones(3)*kp)
B = np.diag(np.ones(3)*kd)
mu =0.3 
# PARAMETERS OF REFERENCE SINUSOIDAL TRAJECTORY
x0          = np.array([0.632, 0.091, 0.472]).T         # offset
f0          = np.array([0.632, 0.091, 0.472]).T         # offset
x_amp         = np.array([0.1, 0.1, 0.0]).T           # amplitude
f_amp         = np.array([0.1, 0.1, 0.0]).T           # amplitude
x_phi         = np.array([0.0, 0.5*np.pi, 0.0]).T     # phase
f_phi         = np.array([0.0, 0.5*np.pi, 0.0]).T     # phase
x_freq        = np.array([0.5, 0.5, 0.3]).T           # frequency (time 2 PI)
f_freq        = np.array([0.5, 0.5, 0.3]).T           # frequency (time 2 PI)
