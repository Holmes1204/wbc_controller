...
for i in range(0, N):
    ...
    x_ref[:,i]  =  np.array([-0.5, -0.5, 0.5])  
    dx_ref[:,i] = np.array([0.0, 0.0, 0.0])  
    ddx_ref[:,i] = np.array([0.0, 0.0, 0.0])  
    ...
    h = robot.nle(q[:,i], v[:,i], False)#include gravity force
    g = robot.gravity(q[:,i])
...
#    dx[:,i] = J @ v[:,i]
    ...
    dJ6 = robot.frameJacobianTimeVariation(q[:,i],v[:,i],frame_id)
    dJ = dJ6[:3,:]
    ...
    # tau[:,i] = solution.task_qp(q[:,i],v[:,i], ddx_des[:,i],M,h,J,dJdq,conf)
    # tau[:,i] = solution.task_cvxopt(q[:,i],v[:,i], ddx_des[:,i],M,h,J,dJdq,conf)
    # tau[:,i] = solution.task_osqp(q[:,i],v[:,i], ddx_des[:,i],M,h,J,dJdq,conf)

    # send joint torques to simulator
    simu.simulate(tau[:,i], conf.dt, conf.ndt)
    tau_c[:,i] = simu.tau_c#friction
